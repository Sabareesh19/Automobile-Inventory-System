...
/* FindReviews class is used to bring the information from the Mongo db based on the conditions specified in the DataAnalytics Ui */
...
public class FindReviews extends HttpServlet {
	...
	protected void doPost(HttpServletRequest request,
    	...
    	String maxopt = request.getParameter("maxOpt");
		...
		if(filters != null && groupBy != true){
			for (int i = 0; i < filters.length; i++) {
				...
				switch (filters[i]){										
					...
					case "max":
					System.out.println("Inside max case: "+maxopt);
						if(maxopt.equals("productPrice")){
							filterByCity = true;
						}
						break;			
					default:
			...
			if(groupBy == true){		
				...
				if(groupByCity){
					System.out.println("Hello I am inside group by city");
			...
			for (DBObject result : aggregate.results()) {
				...
				while (detailcount < productList.size()) {
					...
					tableData = "<tr rowspan = \"3\"><td> Product: "+productList.get(detailcount)+"</br>"
							...
							+   "retailerzipcode: "+zipcode.get(detailcount)+"</br></br>"
