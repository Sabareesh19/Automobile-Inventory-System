...
/* Registration java file is used to create a new user,Admin */
	...
	protected void displayRegistration(HttpServletRequest request,
		...
		throws ServletException, IOException {
			...
			pw.print("<form method='post' action='Registration'>"
					...
					+ "</td></tr><tr><td>");
			if(utility.isLoggedin()){
				pw.print("<h3>User Type</h3></td><td><select name='usertype' class='input'><option value='customer' selected>Customer</option></select>"
					+ "</td></tr></table>"
				...
				pw.print("<h3>User Type</h3></td><td><select name='usertype' class='input'><option value='customer' selected>Customer</option><option value='manager'>Store Manager</option></select>"
					+ "</td></tr></table>"
					+ "<input type='submit' class='btnbuy' name='ByUser' value='Create User' style='float: right;height: 20px margin: 20px; margin-right: 10px;'></input>"
					+ "</form>" + "</div></div></div>");
			}
